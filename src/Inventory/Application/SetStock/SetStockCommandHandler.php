...
final readonly class SetStockCommandHandler
{
     ...
     * whole request is rejected before any write (all or nothing); the write
     * itself is a single atomic, concurrency-safe upsert (see StockRepository).
     *
    ...
    public function __invoke(SetStockCommand $command): array
    {
        ...
        $upserted = array_map(
            static fn (StockLine $line): Stock => Stock::create(
                StockId::generate(),
                $productId,
                ShopId::fromString($line->shopId),
                new Quantity($line->quantity),
            ),
            $command->lines,
        );

        $this->stocks->upsertAll($upserted);
